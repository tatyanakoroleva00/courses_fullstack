...
$query = "SELECT DISTINCT last_name
FROM authors;";
...
} else if ( isset($_POST['filter']) && $_POST['filter'] === 'authors' ) {
    echo $_POST['authors'];
    $author = htmlspecialchars(trim($_POST['authors']));
    $query = "SELECT news.`id` AS news_id, `category`, `title`, `short_text`, `news_image`, `add_date`, 
          authors.id AS author_id, first_name, last_name 
					FROM `news`, authors 
					WHERE authors.id = author_id
...
<div class="news">
    ...
    <form method="POST">
        <label>Фильтр по авторам: </label><br>
        <select name="authors" multiple>
            <option value="">--Выберите автора:--</option>
            <?php foreach($authors as $author):?>
                <option value="<?=$author['last_name']?>"><?=$author['last_name']?></option>
            <?php endforeach;?>
        </select><br>
        <input type="hidden" name="filter" value="authors">
        <input type="submit" value="Выбрать">
    </form>
